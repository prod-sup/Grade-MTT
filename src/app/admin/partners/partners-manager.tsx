  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        {partners.length === 0 ? (
          <p
            className={`rounded-lg border border-dashed border-gray-300 dark:border-white/[0.1] px-4 py-8 text-center text-sm ${TEXT_MUTED}`}
          >
...
function InviteForm({ onChanged }: { onChanged: () => void }) {
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 rounded-lg border ${BORDER_SUBTLE} ${SURFACE_ELEVATED_BG} px-3 py-3`}>
      <span className={`text-sm ${TEXT_SECONDARY}`}>Convidar parceiro:</span>
      <input
        type="email"
        value={email}
        onChange={(e) => setEmail(e.target.value)}
        placeholder="email@parceiro.com"
        className={`min-w-[16rem] rounded-lg border border-gray-200 dark:border-white/[0.12] bg-gray-50 dark:bg-white/[0.03] px-2 py-1.5 text-sm ${TEXT_PRIMARY} outline-none focus:border-gray-400 dark:focus:border-white/[0.3]`}
      />
      <button
        type="button"
        onClick={submit}
...
function InviteRowItem({ invite, onChanged }: { invite: InviteRow; onChanged: () => void }) {
  ...
  return (
    <div className={`flex items-center justify-between gap-2 rounded-lg border ${BORDER_SUBTLE} px-3 py-2 text-sm`}>
      <span className={TEXT_BODY}>{invite.email}</span>
      ...
      <button
        ...
        onClick={resend}
        disabled={pending}
        className={`rounded border border-gray-200 dark:border-white/[0.12] px-2 py-1 text-xs ${TEXT_BODY} hover:border-gray-300 dark:hover:border-white/[0.2] hover:bg-gray-50 dark:hover:bg-white/[0.05] disabled:opacity-50`}
      >
...
function PartnerCard({ partner, onChanged }: { partner: PartnerRow; onChanged: () => void }) {
  ...
  return (
    <div className={`flex flex-col gap-3 rounded-lg border ${BORDER_SUBTLE} ${SURFACE_ELEVATED_BG} p-4`}>
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className={`font-medium ${TEXT_PRIMARY}`}>{partner.clubName}</p>
          <p className={`text-xs ${TEXT_MUTED}`}>
            {partner.contactName} · {partner.email}
          </p>
        </div>
        <label className={`flex items-center gap-1.5 text-xs ${TEXT_SECONDARY}`}>
          <input
            type="checkbox"
      ...
      {partner.brand ? (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2">
            ...
            {partner.brand.phoneText ? (
              <span className={`text-xs ${TEXT_MUTED}`}>{partner.brand.phoneText}</span>
            ) : null}
          </div>
          ...
          <div className="flex gap-2">
            {partner.brand.logoUrl ? (
              ...
              <img
                ...
                alt="Logo"
                className={`h-12 w-12 rounded border ${BORDER_SUBTLE} bg-black/30 object-contain`}
              />
            ) : null}
            ...
            rejecting ? (
              <div className="flex flex-col gap-2">
                <textarea
                  ...
                  className={`rounded-lg border border-gray-200 dark:border-white/[0.12] bg-gray-50 dark:bg-white/[0.03] px-2 py-1.5 text-sm ${TEXT_PRIMARY} outline-none focus:border-gray-400 dark:focus:border-white/[0.3]`}
                  rows={2}
                />
                <div className="flex gap-2">
                  <button
                  ...
                  <button
                    ...
                    className={`rounded-lg border border-gray-200 dark:border-white/[0.12] px-3 py-1.5 text-xs ${TEXT_BODY} hover:bg-gray-50 dark:hover:bg-white/[0.05]`}
                  >
                    Cancelar
                  </button>
                </div>
        ...
        <p className={`text-xs ${TEXT_MUTED}`}>Ainda não enviou a marca.</p>
      )}
